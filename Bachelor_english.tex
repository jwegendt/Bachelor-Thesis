...
I discuss the decidable subproblem of first-order unification and an accompanying algorithm adapted to our syntax.
Furthermore, I explain higher-order unification and the specific problem of pattern unification formally.

Finally, algorithm for the presented calculus and higher-order unification is presented, 
which covers all pattern unification problems and those which can be reduced to patterns.
...
I will give examples and show how we can use this syntax to define everything and point out how the duality plays out in our syntax.
Furthermore, I will introduce some concepts we need to play around with terms.

In \ref{sec:Simple Types}, I give examples of how we can type terms we have seen in our examples, 
and present a typing system to categorize many terms in simple types.

To make our way to higher-order unification, I will first discuss \ref{sec:First-Order Unification},
explaining the problem formally and giving an algorithm to solve it.

Then I explain \ref{sec:Higher-Order Unification} formally, as well as a subproblem which is easier to solve.
...
In the end (\ref{sec:Literature review}) is a literature review, going over connected works.
...
\begin{example}[Using pattern matching to take apart data]
    \begin{align*}
        ...
        &\mathtt{Cons(True, Cons(False, Nil))}.\expatmatch{\mathttbrackets{Cons}{x, y}\Rightarrow x}\tag{2}   
    \end{align*}
    This is the pattern matching you know and love. 
    We will see later (~\ref{sec:betaconv}) how these get evaluated to $\mathtt{1}$ and $\True$, respectively.
...
\begin{example}[Substitutions on unification variables] 
    ...
    \end{align*}     
    If we now found the solution $\sigma = [x/ \alphaunifvar]$ through another equation, we would actually need to substitute in $\alphaunifvar$!
\end{example}
...
%TODO: das hier nochmal überdenken+löschen?
...
\begin{example}[Unification]
    ...
    \begin{align*}
        ...
        \vdash \betaunifvar\equiv \alphaunifvar.\expatmatch{\mathtt{T \Rightarrow 1, F\Rightarrow 5}}\footnotemark{}\tag{1}\\
...
%TODO: is it possible to introduce new constraints that we can simplify? probably right? 
