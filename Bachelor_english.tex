...
I expand on this in \cref{sec:The Algorithm}. 
\end{definition}
...
\subsection{The Algorithm}\label{sec:The Algorithm}
...
When we can't simplify anymore, we start to apply unification steps.
...
\begin{example}[Applying \cref{unif4} too early]
    ...
    \end{align*}
    If we could chose what unification steps to apply when, we could now apply \cref{unif5} and conclude that there is no solution.
    ...
    \end{align*}
    and conclude with the following solution: $[\mathtt{5, Tup(5, 3)}/\alphaunifvar, \betaunifvar]$.
