...
Note that this is encompassed our syntax described ~\ref{sec:syntax},
...
In the introduction (~\ref{sec:mgu}), I alluded to problems with multiple solutions having one solution we prefer.
...
Why do we need a formal definition here? Because we don't solve our unification problem in one huge step, but
...
\begin{example}[Pattern and Copattern Matching]
    ...
    \\
    &\excopatmatch{\mathtt{fst \Rightarrow} \mathtt{5}, \mathtt{snd\Rightarrow False}}\equiv \alphaunifvar
...
\begin{definition}[Decomposing Constraints]
    ...
    \tag{2}\\
    &\Psi \vdash \exconstructor{} \equiv \exconstructor{} &\mapsto_c& \top 
    \tag{3}\\
    &\textbf{Decomposition}\\
...
If any of this does not hold in a given equation, rule 12 applies and we simplify to $\bot$.
