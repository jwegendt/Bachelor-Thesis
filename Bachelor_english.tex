...
Finally, I present a framework for our algorithm, as well as a unification algorithm for the aforementioned calculus. 
...
These are the goals for the next few paragraphs, after which I give a quick overview of the different parts of this thesis.
...
\begin{definition}[Terms of the Calculus ND]
    ...
    In $e.\expatmatch{\mathttbrackets{Tup}{x_1, x_2}\Rightarrow t_1, \mathttbrackets{Tup}{x, y}\Rightarrow t_2}$, the same tuple could be reduced to two different terms, $t_1$ and $t_2$.
\end{definition}
...
If we construct terms, we naturally also want a way to take them apart. 
...
\begin{example}[Instantiating codata with copattern matching]
    ...
    The second example is the infinite stream of $Ts$.
\end{example}
...
We need a couple of concepts to describe the processes needed in unification algorithms. 
...
\begin{example}[Idempotency]
    $[\mathtt{Tup(5, 7)}/x]$ is idempotent, since:
    \begin{align*}
        &[\mathtt{Tup(5, 7)}/x] \circ [\mathtt{Tup(5, 7)}/x]\\
        =& [\mathtt{Tup(5[\mathtt{Tup(5, 7)}/x], 7[\mathtt{Tup(5, 7)}/x])}/x]\\
        =& [\mathtt{Tup(5, 7)}/x]
    \end{align*}
    ...
    \end{align*}
\end{example} %TODO: interessanteres beispiel??
...
\begin{example}[Basic types]
    \begin{align*}
        \mathtt{F}&: \Bool\\
        \mathtt{Tup(1,T)}&: \mathsf{Pair(Int, Bool)}\\
        \excopatmatch{\mathtt{fst \Rightarrow 1, snd\Rightarrow T}}&: \mathsf{LPair(Int, Bool)}\\
        \mathtt{Cons(1, Cons(2, Nil))}&: \mathsf{List(Int)}\\
        Ts = \excopatmatch{\mathtt{hd \Rightarrow T, tl \Rightarrow} Ts}&: \mathsf{Stream(Bool)}\\
        \excopatmatch{\ap{x}\Rightarrow x.\expatmatch{\mathtt{T\Rightarrow F, F \Rightarrow T}}}&: \mathsf{Bool \rightarrow Bool}
    \end{align*}    
\end{example}
...
\begin{minipage}{0.35\textwidth}
    \begin{prooftree}
        \AxiomC{}\RightLabel{\textsc{T}}
        \UnaryInfC{$\Gamma\vdash\mathtt{T}:\mathsf{Bool}$}  
    \end{prooftree}
\end{minipage}
\begin{minipage}{0.35\textwidth}
    \begin{prooftree}
        \AxiomC{}\RightLabel{\textsc{F}}
        \UnaryInfC{$\Gamma\vdash\mathtt{F}:\mathsf{Bool}$}  
    \end{prooftree}
\end{minipage}
...
\begin{theorem}[Decidability of First-Order Unification]
    ...
    In particular, this solution is always a mgu i.e. if there is a solution, then it is always the most general one.
...
\begin{definition}[Higher-Order Unification]
    ...
    \begin{align*}
        e,t  ::&= \alphaunifvar\sigma \tag*{Unification variable with substitution}\\ %TODO: name? 
            &\partition x  \tag*{Variable} \\
            &\partition\constructor \tag*{Constructor} \\
            &\partition\patmatch  \tag*{Pattern match}\\
            &\partition\copatmatch  \tag*{Copattern match}\\
...
\begin{example}[Pattern and Copattern Matching]
    ...
    &\vdash\excopatmatch{\mathtt{fst \Rightarrow} \mathtt{5}, \mathtt{snd\Rightarrow F}}\equiv \alphaunifvar \tag{3}
    \\ &\mapsto \vdash \mathtt{5}\equiv \alphaunifvar.\mathtt{fst}\, \wedge \,\vdash \mathtt{F\equiv\alphaunifvar.snd}
    \end{align*}
\end{example}
...
\begin{example}[Removing contradictions in $\circ$-equations]
    \begin{align}
...
\begin{example}[Removing contradictions in *-equations]
    \begin{align*}
...
\begin{align*} %overfull
    \constraints &= &\vdash\ & \mathtt{Tup(\alphaunifvar, F)}\equiv \mathtt{Tup(T, F)}&\wedge&
    \vdash \betaunifvar\equiv \alphaunifvar.\expatmatch{\mathtt{T \Rightarrow 1, F\Rightarrow 5}}\\
    &\xmapsto{(5)}_d &\vdash\ & \mathtt{\alphaunifvar\equiv T}\qquad\wedge\vdash \mathtt{F\equiv F} &\wedge& 
    \vdash \betaunifvar\equiv \alphaunifvar.\expatmatch{\mathtt{T \Rightarrow 1, F\Rightarrow 5}}\\
...
\begin{align*} %split lines
    \constraints &= \vdash\ \mathtt{Tup(\alphaunifvar, F)}\equiv \mathtt{Tup(T, F)}\\
    &\wedge\vdash \betaunifvar\equiv \alphaunifvar.\expatmatch{\mathtt{T \Rightarrow 1, F\Rightarrow 5}}\\
    &\xmapsto{(5)}_d \vdash\   \mathtt{\alphaunifvar\equiv T}\qquad\wedge\vdash \mathtt{F\equiv F}\\
    &\wedge\vdash \betaunifvar\equiv \alphaunifvar.\expatmatch{\mathtt{T \Rightarrow 1, F\Rightarrow 5}}\\
    &\xmapsto{(3)}_r \vdash\   \mathtt{\alphaunifvar\equiv T} \\
    &\wedge\vdash \betaunifvar\equiv \alphaunifvar.\expatmatch{\mathtt{T \Rightarrow 1, F\Rightarrow 5}}\\
    &\mapsto_u \vdash\  \mathtt{\alphaunifvar\equiv T}\\
    &\wedge\vdash \betaunifvar\equiv \mathtt{T}.\expatmatch{\mathtt{T \Rightarrow 1, F\Rightarrow 5}}\\
    &\stackrel{\betaconv}{=} \vdash\  \mathtt{\alphaunifvar\equiv T} \\
...
\begin{example}[Introducing simplifiable constraints through unificiation steps]
    \begin{align*}
...
Nonetheless, the author described its duality to data and gives a theoretical overview of codata. %TODO: he does more than that! but i dont know how to describe it in a sentence. is this relevant?
%codata in action
...
where he also goes into more depth on 
