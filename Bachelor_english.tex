...
I will introduce the Untyped Calculus ND, based on .... %TODO: citation 
...
in many applications it is all that is needed --- most simple types can be constructed from first-order terms. 
...
\begin{definition}[Unification algorithm for First-Order Unification ]
    ...
    \begin{align*}
        ...
        E & \cup \{\alphaunifvar \equiv K(\overline{e})\}\Rightarrow \perp \quad \text{if $\alphaunifvar \in \overline{e}$} \tag{occurs check} \label{a}
    \end{align*}
\end{definition}
...
is a subset of higher-order unification problems, and its path to a solution is similarly simple as the one to first-order unification.
...
This means that the pattern fragment described by Miller is a subset of our definition.
...
Equations that contain first-order terms, but no patterns still remain solvable.
...
This is what is called \textbf{dynamic pattern unification}, where we hold off on solving some equations 
...
\begin{definition}[Constraint]
    ...
    \end{align*}
    Where $\top$ and $\bot$ are the symbols for statements that are trivially true or false. 
...
In summary, I presented a higher-order unification algorithm solving pattern unification for languages with a strong data-codata duality.
