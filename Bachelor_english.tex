...
\begin{theorem}[Decidability Of First-order Unification]
    There exists an algorithm which always terminates, and returns the solution if there exists one. 
...
\begin{theorem}[Decidability of Higher-Order Unification]
    In higher-order unification includes unification problems containing higher-order terms (equivalent to lambda abstractions),
    and is covered by our introduced syntax. %TODO: bessere formulierung?
    Higher-order unification is not decidable.
\end{theorem}

\section{The Pattern Fragment}

The pattern fragment is a subset of higher order unification problems for which mgus exist. 
