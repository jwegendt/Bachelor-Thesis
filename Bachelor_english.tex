...
\texttt{cons(True, cons(False, Nil))} and \texttt{Date(27, 08, 2005)} 
...
When writing $\Gamma\vdash t:\tau$, we mean that from the variables in $\Gamma$, 
...
\begin{prooftree}
    ...
    \UnaryInfC{$\Gamma\vdash Nil:List(\tau)$}
\end{prooftree}
\begin{prooftree}
    \AxiomC{$\Gamma\vdash t_1:\tau$}\RightLabel{\textsc{Cons}}
    \AxiomC{$\Gamma\vdash t_2:List(\tau)$}
    \BinaryInfC{$\Gamma\vdash Cons(t_1,t_2):List(\tau)$}
\end{prooftree}
\begin{prooftree}
    \AxiomC{$\Gamma\vdash t:List(\tau')$}\RightLabel{\textsc{Case-List}}
    \AxiomC{$\Gamma\vdash t_1:\tau$}
    \AxiomC{$\Gamma, y:\tau',z:List(\tau')\vdash t_2:\tau$}
    \TrinaryInfC{$\Gamma\vdash t.\expatmatch{Nil\Rightarrow t_1, Cons(y,z)\Rightarrow t_2}:\tau$}
\end{prooftree}
\begin{prooftree}
    \AxiomC{$\Gamma:t:Stream(\tau)$}\RightLabel{\textsc{Hd}}
    \UnaryInfC{$\Gamma\vdash t.hd:\tau$}    
\end{prooftree}
\begin{prooftree}
    \AxiomC{$\Gamma\vdash t:Stream(\tau)$}\RightLabel{\textsc{Tl}}
    \UnaryInfC{$\Gamma\vdash t.tl:Stream(\tau)$}
\end{prooftree}
\begin{prooftree}
    \AxiomC{$\Gamma\vdash t_1:\tau$}\RightLabel{\textsc{Stream}}
    \AxiomC{$\Gamma\vdash t_2:Stream(\tau)$}
    \BinaryInfC{$\Gamma\vdash \excopatmatch{hd\Rightarrow t_1, tl\Rightarrow t_2}:Stream(\tau)$}
\end{prooftree}
\begin{prooftree}
    \AxiomC{$\Gamma\vdash t_1:\tau_1$}\RightLabel{\textsc{Lpair}}
    \AxiomC{$\Gamma\vdash t_2:\tau_2$}
    \BinaryInfC{$\Gamma\vdash t.\expatmatch{fst\Rightarrow t_1, snd \Rightarrow t_2}:LPair(\tau_1, \tau_2)$}
\end{prooftree}
\begin{prooftree}
...
\begin{prooftree}
    ...
    \UnaryInfC{$\Gamma\vdash\excopatmatch{\Ap{x}\Rightarrow t}:\tau_1\rightarrow t_2$}
\end{prooftree}
...
\begin{align*}
    \begin{split}
        ...
        s &= \excopatmatch{\Ap{x}\Rightarrow \texttt{cons(x cons(y Nil))}},   \\
        t &= \alphaunifvar.\Ap{x_1, x_2, x_3}
    \end{split}    
\end{align*}
...
As an example, the problem $\texttt{True}.\textbf{case} \{ \texttt{True} => a?, \texttt{False} => 2 \} \equiv 3 $ has the solution $[3/\alphaunifvar]$, 
...
\begin{example}[Decomposing constraints-redundancy and contradiction]
    ...
    \end{align*}
\end{example}
These example might be confusing at first. Looking at the third example, one might argue that there exists a pair, say $x = \texttt{Pair(}2, 2\texttt{)}$, such that
$x.\pi_1 = x.\pi_2$. This is where it is important to remember that we are not looking for solutions to variables like $x$.
...
The equation $x \vdash x.\pi_1 \equiv x.\pi_2$ must hold for \textit{any} variable $x$, since we cannot choose $x$! 
%TODO: rework sentence structure
%TODO: can i explain this better?

Since for $x = \texttt{Pair(}2, 3\texttt{)}$, the equation does not hold, we can simplify to $\bot$.
