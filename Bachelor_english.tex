...
While data is a well-known concept and computer scientist have spent a considerate amount thinking about it, 
...
For example, look at these different ways to define the same tuple: 
$\mathttbrackets{Tup}{1,5}$ vs. $\excopatmatch{fst\Rightarrow \mathtt{1}, snd \Rightarrow \mathtt{5}}$
While the first focuses on what we put into the tuple and its construction, the second focuses on what happens when we take its components out of it, destroy it.

This is something still different from Copatterns. Copatterns 
...
$\mathtt{5} \equiv \alphaunifvar$ and $\mathtt{True \equiv False}$ are examples for first-order unification, 
where the first has the solution $\alphaunifvar = \mathtt{5}$ and the second is unsolvable.
$\mathsf{List(\alphaunifvar)\equiv List(\betaunifvar)}$ is a unification problem on types and has many solutions.

In higher-order unification, functions may appear in our equations. This can look like this:
$\alphaunifvar.\Ap{\mathtt{5}} \equiv \mathtt{5}$, which has the solution $\alphaunifvar = id$ 
%TODO: ap erklären oder ist das offensichtlich durch die identitätsfunktion?
...
\begin{minipage}{0.45\textwidth}
    \begin{prooftree}
        \AxiomC{$\Gamma\vdash t:\mathsf{Lpair(tau_1,tau_2)}$}\RightLabel{\textsc{Snd}}
...
Some of these example might be confusing at first. Looking at the third example, one might argue that there exists a pair, say $x = \mathtt{Tup(2,2)}$, such that
...
When we have found a solution for each unification variable, we can terminate and give the solution. 
