...
In higher-order unification, functions may appear in our equations. This can look like:
...
All rudimentary datatypes can be constructed through constructors. Constructors are also used for recursively data types like numbers (peano numbers) or lists.
%TODO: explain that im using a shorthand for peano numbers => fußnote?
\begin{example}[Instantiating Data Types with constructors]
    \begin{align*}
        &\mathtt{True, False}\\
        &\mathtt{Tup(1,2)}\\
...
We also use constructos for types, Simple Types like $\mathsf{Int, Bool, ...}$ are constructors on an empty sequence, 
while $\mathsf{List(Int)}$ is a constructor that takes one argument, in this case $\mathsf{Int}$. 

\begin{example}[Constructing Type with constructors]
    \begin{align*}
        &\mathsf{Int}\\
        &\mathsf{Bool}\\
        &\mathsf{List(Int)}
