...
\begin{example}[Pattern and Copattern Matching]
    ...
    &\vdash\excopatmatch{\mathtt{fst \Rightarrow} \mathtt{5}, \mathtt{snd\Rightarrow False}}\equiv \alphaunifvar \tag{3}
...
\begin{definition}[Decomposing Constraints]
    ...
    *\, &\Psi\vdash \excopatmatch{\overline{d(\overline{x})\Rightarrow e}}\equiv 
    ...
    \tag{10}\\
    &\text{non-matching con-/destructors in $\circ$-equation}&\mapsto_c&\bot\tag{11}\\
    &\text{non-matching con-/destructors or non-matching}\\
    &\text{variables in *-equation} &\mapsto_c& \bot
    \tag{12}\label{12}
    \end{align*}
\end{definition}
...
In *-equations where the arguments are not equal syntactically, rule (\ref{12}) applies and we simplify to $\bot$:
